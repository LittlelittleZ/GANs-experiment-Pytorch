...
def cal_IS(model, dim_z, bs=50, n_total=5000, splits=1, resize=True):
    model.eval()
    inception = incepnets.inception_v3(pretrained=True)
    inception.eval()
    ...
    for i in range(n_total // bs):
        z = Variable(torch.randn(bs, dim_z))
...
if __name__ == '__main__':
    ...
    net = Generator(3)
    net.load_state_dict(torch.load('1_G.pth'))
    net
    print('IS:{}'.format(cal_IS(net, dim_z=100, n_total=25000, splits=5)))

    dataset = dset.CIFAR10(root='../cifar-10-batches-py', download=True,
