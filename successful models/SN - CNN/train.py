...
if __name__ == "__main__":
    ...
    for epoch in range(max_step):
        ...
        utils.save_image(grid, model_dir['samples'] + '/f{}_SN_RES.png'.format(epoch), nrow=g_batch_size , normalize=True)
        print('[%d/%d] D(x): %.4f D(G(z)): %.4f/ %.4f'% (epoch, 350, D_x, D_G_z1, D_G_z2))
        # print('alpha: ', model.alpha)
        print("{:.4f} minutes...".format((time_end - time_start).seconds / 60.))
        
        if epoch % 25 == 25 - 1:
            torch.save(model.G.state_dict(), model_dir['checkpoint'] + "/{}_SN_RES_G.pth".format(epoch+1))
            torch.save(model.D.state_dict(), model_dir['checkpoint'] +"/{}_SN_RES_D.pth".format(epoch + 1))
