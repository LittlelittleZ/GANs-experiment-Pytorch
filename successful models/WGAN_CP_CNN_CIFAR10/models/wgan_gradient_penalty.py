    ...
    def train(self, train_loader):
        ...
        for g_iter in range(self.generator_iters):
            ...
            if (g_iter) % 1000 == 0:
                ...
                if not os.path.exists('wgangp_cifar_training_result_images/'):
                    os.makedirs('wgangp_cifar_training_result_images/')
                ...
                utils.save_image(grid,
                                 'wgangp_cifar_training_result_images/wgangp_cifar_img_generatori_iter_{}.png'.format(
                                     str(g_iter).zfill(3)))

                # Testing
                time = t.time() - self.t_begin
                # print("Real Inception score: {}".format(inception_score))
                print("wgangp_cifar_Generator iter: {}".format(g_iter))
                print("wgangp_cifar_Time {}".format(time))
                ...
                # (3) Log the images
                info = {
                    'wgangp_cifar_real_images': self.real_images(images, self.number_of_images),
                    'wgangp_cifar_generated_images': self.generate_img(z, self.number_of_images)
                }
        ...
        self.t_end = t.time()
        print('Time of WGAN_GP Cifar training-{}'.format((self.t_end - self.t_begin)))
        # self.file.close()
