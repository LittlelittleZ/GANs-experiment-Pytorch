...
class WGAN_CP(object):
    def __init__(self, args):
        ...
        self.generator_iters = args.generator_iters  # 生成器的迭代次数
        self.critic_iter = 5
    ...
    def train(self, train_loader):
        ...
        for g_iter in range(self.generator_iters):
            ...
            if (g_iter) % 1000 == 0:
                ...
                if not os.path.exists('wgancp_cifar_training_result_images/'):
                    os.makedirs('wgancp_cifar_training_result_images/')
                ...
                utils.save_image(grid,
                                 'wgancp_cifar_training_result_images/wgancp_cifar_img_generatori_iter_{}.png'.format(
                                     str(g_iter).zfill(3)))

                # Testing
                time = t.time() - self.t_begin
                # print("Inception score: {}".format(inception_score))
                print("wgancp_cifar_Generator iter: {}".format(g_iter))
                print("wgancp_cifar_Time {}".format(time))
                ...
                # (3) Log the images
                info = {
                    'wgancp_cifar_real_images': self.real_images(images, self.number_of_images),
                    'wgancp_cifar_generated_images': self.generate_img(z, self.number_of_images)
                }
        ...
        self.t_end = t.time()
        print('Time of WGAN_CP Cifar training-{}'.format((self.t_end - self.t_begin)))
        # self.file.close()
    ...
    def save_model(self, g_iter):
        torch.save(self.G.state_dict(), './checkpoint' + "/{}_G.pth".format(g_iter + 1))
        torch.save(self.D.state_dict(), './checkpoint' + "/{}_D.pth".format(g_iter + 1))
